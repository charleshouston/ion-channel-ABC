### Probability distributions for ABC.

"""
Originally written by Aidan Daly, DPhil candidate, University of Oxford
Department of Computer Science as part of a paper tutorial on ABC
parameter estimation for the Hodgkin-Huxley action potential model.
Adapted for use in the current project with myokit.
"""
...
class Normal(AbstractDistribution):
    def __init__(self, mean=0.0, var=1.0):
        ...
        self.var = var
    def pdf(self, x):
        return (1 / (math.sqrt(2*math.pi*self.var))) * math.exp(
                -pow(x - self.mean, 2) / (2 * self.var))
    def draw(self):
